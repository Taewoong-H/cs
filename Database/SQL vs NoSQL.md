...
따라서 위 사진에 SQL에서 진행한 Orders, Users, Products 테이블로 나눈 것을 NoSQL에서는 Orders에 한꺼번에 포함헤서 저장하게 된다.
...
데이터 저장방식으로 SQL DB는 일반적으로 수직적 확장만 지원한다.
수평적 확장은 NoSQL DB에서만 가능하다.
둘중 뭘 선택하느냐에 대한 정답은 없다. 어떤 데이터를 다루느냐에 따라 원하는 방식에 맞게 선택을 고려하면 된다.
</br>
...
- 관계를 맺고 있어서 조인문이 많은 복잡한 쿼리가 만들어 질 수 있듬.
- 대체로 수직적 확장만 가능하다.
  </br>
